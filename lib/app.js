...
app.get('/kpop', async(req, res) => {
  try {
    const data = await client.query('SELECT * from kpop');
...
app.get('/kpop/:id', async(req, res) => {
  try {
    const data = await client.query('SELECT * from kpop where id=$1', [req.params.id]);
...
app.post('/kpop/', async(req, res) => {
  try {
    const data = await client.query(`
      ...
      VALUES ($1, $2, $3, $4, 1)
      RETURNING *`, [req.body.name, req.body.members, req.body.gender, req.body.debut_year]);
...
app.put('/kpop/:id', async(req, res) => {
  try {
    const data = await client.query(`
      ...
      WHERE id=$5
      RETURNING *
    `, [req.body.name, req.body.members, req.body.gender, req.body.debut_year, req.params.id]);
